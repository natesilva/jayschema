...
// ******************************************************************
// [static] RegExps for use with the format keyword.
// ******************************************************************
TestSet.FORMAT_REGEXPS = {
...
TestSet.prototype.formatDateTime = function() {
  var errors = [];
  var valid = false;

  var match = this.inst.match(TestSet.FORMAT_REGEXPS['date-time']);
  ...
  if (!valid) {
    ...
    errors.push(new Errors.FormatValidationError(this.resolutionScope,
      this.instanceContext, 'format', 'date-time', this.inst, desc));
  }

  return errors;
};

TestSet.prototype.formatEmail = function() {
  var errors = [];
  var valid = this.inst.match(TestSet.FORMAT_REGEXPS.email);
  if (!valid) {
    var desc = 'not a valid email address';
    errors.push(new Errors.FormatValidationError(this.resolutionScope,
      this.instanceContext, 'format', 'email', this.inst, desc));
  }
  return errors;
};

TestSet.prototype.formatHostname = function() {
  var errors = [];

  var valid = this.inst.match(TestSet.FORMAT_REGEXPS.hostname);
  if (valid) {
    // per RFC 1035 “Preferred name syntax” each label must be no
    // more than 63 characters.
    var labels = this.inst.split('.');
  ...
  if (!valid) {
    ...
    errors.push(new Errors.FormatValidationError(this.resolutionScope,
      this.instanceContext, 'format', 'hostname', this.inst, desc));
  }

  return errors;
};

TestSet.prototype.formatIpv4 = function() {
  var errors = [];

  if (!this.inst.match(TestSet.FORMAT_REGEXPS.ipv4)) {
    var desc = 'not a valid dotted-quad IPv4 address';
    errors.push(new Errors.FormatValidationError(this.resolutionScope,
      this.instanceContext, 'format', 'ipv4', this.inst, desc));
  }

  return errors;
};

TestSet.prototype.formatIpv6 = function() {
  var errors = [];
  var index, len;

  // *** form 1
  if (this.inst.match(TestSet.FORMAT_REGEXPS.ipv6.form1)) { return errors; }
  if (this.inst.match(TestSet.FORMAT_REGEXPS.ipv6.form3full)) { return errors; }

  if (this.inst.match(TestSet.FORMAT_REGEXPS.ipv6.allForms)) {
    var parts = this.inst.split(':');
    if (
        parts.length >= 3 &&
        parts.length <= 8 &&
        (
          this.inst.indexOf('.') === -1 ||
          this.inst.indexOf('.') > this.inst.lastIndexOf(':')
        )
       )
    {

      if (this.inst.indexOf('::') !== -1) {
        ...
        var expanded = this.inst.replace('::', replacement);
        if (expanded[0] === ':') { expanded = expanded.slice(1); }
        if (expanded.slice(-1) === ':') { expanded = expanded.slice(0, -1); }

        if (expanded.match(TestSet.FORMAT_REGEXPS.ipv6.form1)) {
          return errors;
        } else if (expanded.match(TestSet.FORMAT_REGEXPS.ipv6.form3full)) {
          return errors;
        }
      }
    }
  ...
  errors.push(new Errors.FormatValidationError(this.resolutionScope,
    this.instanceContext, 'format', 'ipv6', this.inst, desc));

  return errors;
};

TestSet.prototype.formatUri = function() {
  var errors = [];

  if (!this.inst.match(TestSet.FORMAT_REGEXPS.uri)) {
...
TestSet.prototype.format = function() {
  var errors = [];

  switch (this.schema.format) {
    case 'date-time':
      errors = errors.concat(this.formatDateTime());
      break;

    case 'email':
      errors = errors.concat(this.formatEmail());
      break;

    case 'hostname':
      errors = errors.concat(this.formatHostname());
      break;

    case 'ipv4':
      errors = errors.concat(this.formatIpv4());
      break;

    case 'ipv6':
      errors = errors.concat(this.formatIpv6());
      break;

    case 'uri':
      errors = errors.concat(this.formatUri());
      break;
  }
