...
JaySchema.prototype.register = function(schema, id, _resolutionScope, _path) {
  var index, len, baseUri, missing;

  if (typeof schema !== 'object' || Array.isArray(schema)) { return []; }

  if (id) {
    baseUri = JaySchema._getBaseUri(id);
    if (this._schemas.hasOwnProperty(baseUri)) {
      var result = [];
      missing = this._schemas[baseUri].missing;
      for (index = 0, len = missing.length; index !== len; ++index) {
        if (!this._schemas.hasOwnProperty(missing[index])) {
          result.push(missing[index]);
        }
      }
      return result;
    }
  ...
  if (id) {
    ...
    if (isFragment) {
      ...
      // top-level schema
      if (!this._schemas.hasOwnProperty(baseUri)) {
        this._schemas[baseUri] = { schema: schema, fragments: {}, missing: {} };
        _path = '#';
  ...
  if (JaySchema._hasEmbeddedIds(schema) || JaySchema._gatherRefs(schema).length)
  {
  ...
  if (this._schemas.hasOwnProperty(baseUri)) {
    this._schemas[baseUri].missing = Object.keys(missing);
    return this._schemas[baseUri].missing;
  } else {
...
// ******************************************************************
// [static] Helper returns true if the object or any of its
// properties contains an 'id' property.
// ******************************************************************
JaySchema._hasEmbeddedIds = function(obj) {
  var currentObj = obj;
  var subObjects = [];

  do {

    if (currentObj.hasOwnProperty('id')) { return true; }
  ...
  instanceContext)
{
  ...
  var baseUri = JaySchema._getBaseUri(schemaId);
  if (!this._schemas.hasOwnProperty(baseUri)) {
